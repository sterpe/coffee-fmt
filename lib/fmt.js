...
PADDED_LR_TYPES = ["UNARY", "LOGIC", "SHIFT", "COMPARE", "COMPOUND_ASSIGN", "MATH", "RELATION", "FORIN", "FOROF", "INSTANCEOF", "UNLESS", "IF", "THEN", "ELSE", "OWN", "WHEN", "LEADING_WHEN", "=", "->", "FOR", "+", "RETURN"];
...
fmt = function(code, options) {
  ...
  _fn = function(i) {
    ...
    if (_ref2 = token[0], __indexOf.call(PADDED_LR_TYPES, _ref2) >= 0) {
      ...
      tmp += token[1];
    } else if (_ref3 = token[0], __indexOf.call([], _ref3) >= 0) {
      if (!formatted_code.charAt(formatted_code.length - 1).match(/\s/)) {
        tmp += " ";
      }
      tmp += token[1];
